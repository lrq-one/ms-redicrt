...
from ms2spectra.utils.misc_utils import np_temp_seed, none_or_nan
from ms2spectra.utils.formula_utils import MASS, ELECTRON_MASS, parse_formula, NEUTRON_MASS
...
def parse_peaks_str(peaks_str):
	# Robust parser for full NIST MSP peak strings.
	# The original code used split(" "), which breaks on multiple spaces/tabs.
	if none_or_nan(peaks_str):
		return np.nan

	peaks = []
	for raw_line in str(peaks_str).splitlines():
		parts = raw_line.strip().split()
		if len(parts) < 2:
			continue

		mz, ints = parts[0], parts[1]
		if mz == "" or ints == "":
			continue
...
def convert_peaks_to_float(peaks):
	# Robust conversion for full NIST MSP.
	# Skip malformed peak entries instead of crashing full preprocessing.
	if not isinstance(peaks, (list, tuple)) or len(peaks) == 0:
		return np.nan
	...
	for peak in peaks:
		...
		float_peaks.append((mz, ints))

	if len(float_peaks) == 0:
		return np.nan
	return float_peaks

def get_res(peaks):
	# Robust resolution calculation.
	if not isinstance(peaks, (list, tuple)) or len(peaks) == 0:
		return np.nan
